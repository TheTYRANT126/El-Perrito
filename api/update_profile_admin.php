...
try {
    ...
    $st = $pdo->prepare("SELECT id_usuario FROM usuario WHERE email = ? AND id_usuario != ?");
    $st->execute([$email, $usuario_id]);
    if ($st->fetch()) {
        $pdo->rollBack();
    ...
    $sql = "UPDATE usuario SET nombre = ?, apellido = ?, email = ?, telefono = ?, direccion = ?";
