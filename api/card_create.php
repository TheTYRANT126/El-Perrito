...
try {
    ...
    if ($es_predeterminada) {
        ...
        // Si no hay tarjetas predeterminadas, marcar esta como predeterminada
        $sql_check = "SELECT COUNT(*) as total FROM tarjeta WHERE id_cliente = ? AND activa = 1 AND es_predeterminada = 1";
    ...
    $sql = "INSERT INTO tarjeta
            (id_cliente, tipo_tarjeta, numero_tarjeta, cvv, nombre_titular, mes_expiracion, anio_expiracion, es_predeterminada)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    $st = $pdo->prepare($sql);
    $st->execute([
        $cid, $tipo_tarjeta, $numero_tarjeta_limpio, $cvv, $nombre_titular, $mes_expiracion, $anio_expiracion, $es_predeterminada
    ]);

    $id_tarjeta = (int)$pdo->lastInsertId();

    $pdo->commit();

    header('Content-Type: application/json; charset=utf-8');
    echo json_encode(['success' => true, 'id_tarjeta' => $id_tarjeta]);

} catch (Exception $e) {
    $pdo->rollBack();
    http_response_code(500);
    echo 'Error al crear la tarjeta';
}
